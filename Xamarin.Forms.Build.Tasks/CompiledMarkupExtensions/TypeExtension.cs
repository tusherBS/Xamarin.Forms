...
namespace Xamarin.Forms.Build.Tasks
{
	class TypeExtension : ICompiledMarkupExtension
	{
		...
		{
			memberRef = module.Import(typeof(Type));
			INode typeNameNode;
			...
			var typeref = module.Import(XmlTypeExtensions.GetTypeReference(valueNode.Value as string, module, node as BaseNode));
			if (typeref == null)
			...
			return new List<Instruction> {
				Instruction.Create(OpCodes.Ldtoken, module.Import(typeref)),
				Instruction.Create(OpCodes.Call, module.Import(getTypeFromHandle))
			};
		}
	}
}