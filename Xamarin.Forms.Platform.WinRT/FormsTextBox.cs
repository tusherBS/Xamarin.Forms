...
#endif
{
	...
	public class FormsTextBox : TextBox
	{
		...
		protected override void OnKeyDown(KeyRoutedEventArgs e)
		{
			if (IsPassword)
			{
				...
				else
				{
					...
					if ((e.Key == VirtualKey.C || e.Key == VirtualKey.X || e.Key == VirtualKey.Z ||  e.Key == VirtualKey.Y) && _ctrlKeyActive)
					{
						e.Handled = false;
						return;
					}
					...
					base.OnKeyDown(e);
					if (_cachedSelectionLength > 0 && !_ctrlKeyActive)
		...
		{
			base.OnKeyUp(e);
			// Track use of Ctrl for OnKeyDown as explained above
			if (e.Key == VirtualKey.Control)
